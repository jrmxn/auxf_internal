function cfg_proc_parent = update_struct_simple(cfg_proc_parent, cfg_proc, verbose)
if nargin < 3
    verbose = false;
end
% GPT4
...
for i = 1:numel(fields)
    ...
    if isstruct(cfg_proc.(currentField))
        ...
        if isfield(cfg_proc_parent, currentField)
            cfg_proc_parent.(currentField) = update_struct_simple(cfg_proc_parent.(currentField), cfg_proc.(currentField), verbose);
        else
